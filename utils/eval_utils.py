...
class PerfEval:
    ...
    def tick(self, label: Optional[str] = None, duration: int = 0):
        self.durations.append(duration)
        if label:
            self.durations_by_label.setdefault(label, []).append(duration)
    ...
    def record_llm_call(
        self, model: str, input_text: str, output_text: str, provider: str = "local"
    ):
        """
    ...
    def get_costs(self):
        ...
        for call in self.llm_calls:
            model = call["model"]
            provider = call.get("provider", "local")
    ...
    def _plot_performance_summary(self, ax, labels, metrics, costs):
        ...
        cached_llm_latency = llm_latency * (1 - hit_rate) + cache_latency * hit_rate
...
Cost/Query: ${costs.get('avg_cost_per_query', 0):.6f}"""
        ...
        summary_text += (
            f"\nOverall latency drop of an LLM app: {int(cached_llm_drop_in_latency * 100)}%\n"
            f"Overall speedup of an LLM app: {cached_llm_speedup:.2f}x"
        )
